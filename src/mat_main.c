...
#ifdef CC_PROTOTYPE_ANSI
int  min_weight_perfect_match (match_edge_t  *edgefilenamePS, match_edge_t *matchPS)
#else
int  min_weight_perfect_match (edgefilenamePS, matchPS)
match_edge_t *edgefilenamePS;
...
//		seed = time (&l);
    ...
    ncount = nnodes_want;
		...
		if (CCutil_getedgelist_nPS (&ncount, edgefilenamePS, &ecount, &elist, &elen)) {
				fprintf (stderr, "getedgelist_n failed\n");
				goto CLEANUP;
			}
...
//    if (perfect_match (ncount, mydat, ecount, &elist, &elen,
		...
		if (perfect_matchPS (ncount, mydat, ecount, &elist, &elen,
                       blossom_file, matchPS, just_frac, no_frac, 
                       use_all_trees, partialprice, &matzeit)) {
			fprintf (stderr, "perfect_match failed\n");
			goto CLEANUP;
    }
