...
static int 
    ...
    write_match (graph *G, CCdatagroup *dat, int *elen, char* match_file),
	  write_matchPS (graph *G, CCdatagroup *dat, int *elen, match_edge_t* matchPS),
...
// printf ("ZZ Edges:  %d (starting)   %d (total)\n", sedges, G.nedges);
    ...
    if (mat_filename !=  (char *) NULL) {
        if (write_match (&G, dat, *elen, mat_filename)) {
...
#ifdef CC_PROTOTYPE_ANSI
int perfect_matchPS (int ncount, CCdatagroup *dat, int ecount, int **elist,
                   int **elen, char *blo_filename, match_edge_t *matchPS,
                   int just_fractional, int no_fractional,
                   int use_all_trees, int partialprice, double *totalzeit)
#else
int perfect_matchPS (ncount, dat, ecount, elist, elen, blo_filename,
                   matchPS, just_fractional, no_fractional,
                   use_all_trees, partialprice, totalzeit)
int ncount;
CCdatagroup *dat;
int ecount;
...
*/
		write_matchPS(&G, dat, *elen, matchPS);
...
#ifdef CC_PROTOTYPE_ANSI
static int write_match (graph *G, CCdatagroup *dat, int *elen, char* fname) 
#else
static int write_match (G, dat, elen, fname) 
graph *G;
CCdatagroup *dat;
int *elen;
char *fname;
...
#ifdef CC_PROTOTYPE_ANSI
static int write_matchPS (graph *G, CCdatagroup *dat, int *elen, match_edge_t *fname) 
#else
static int write_matchPS (G, dat, elen, fname) 
graph *G;
CCdatagroup *dat;
int *elen;
match_edge_t *fname;
...
//    printf(" %i nodes, %i edges ", G->nnodes, c);
	...
	if (i == e->orig_nod2 || e->x == HALF) {
			...
			//fprintf (fp,"%i %i %i\n",
						//       G->edgelist[G->nodelist[i].matched_edge].orig_nod1,
						//       G->edgelist[G->nodelist[i].matched_edge].orig_nod2, len);
						fname[k].a = (long)G->edgelist[G->nodelist[i].matched_edge].orig_nod1;
						fname[k].b = (long)G->edgelist[G->nodelist[i].matched_edge].orig_nod2;
						fname[k].cost = len;
						k++;
}
    }
